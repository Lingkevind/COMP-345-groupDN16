...
Advance::Advance(Player* orderHolder, int army, Territory* source, Territory* target){
	...
	if (source->getArmySize() > target->getArmySize()) {
		target->setArmySize(source->getArmySize() - target->getArmySize());
		source->setArmySize(0);
		target->setControllingPlayer(source->getControllingPlayer());
	}
}
