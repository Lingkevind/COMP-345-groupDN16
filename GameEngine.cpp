...
void PlayersAddedState::executeState(CommandProcessor* cp)
{
	...
	else if (commandString == "gamestart")
	{
		...
		do
		{
			for (Player * currentplayer : this->context_->playerCollection) {
		...
		if (testGameStartup)
		{
			...
			for (Continent * gameC : gameContinents) {
				vector<Territory*> territoryV = gameC->getTerritories();
				for (Territory * var : territoryV) {
					territoriesNum++;
				}
			}
			bool vMe50Army = true;
			for (Player * var : this->context_->playerCollection) {
...
void IssueOrderState::executeState(CommandProcessor* cp)
{
	...
	for (int i = 0; i < pAmount; i++) {
		...
		this->context_->playerCollection[i]->issueDeploy();
	}
	...
	while (turnEnd(endTurnIndicator) == false) {
		for (int i = 0; i < pAmount; i++) {
			cout << "=====================================================" << endl;
			endTurnIndicator[i] = this->context_->playerCollection[i]->issueOrder(&this->context_->deck, this->context_->gameMap);
		}
	}
